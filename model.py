...
class TripletNet(nn.Module):
	def __init__(self,featurextractor):
		super(TripletNet,self).__init__()
		self.featurextractor = featurextractor
		# for param in featurextractor.classifier.parameters():
		# 	print param.requires_grad

	def forward(self, anchor, positive, negative):
		anchor_output = self.featurextractor(anchor)
		positive_output = self.featurextractor(positive)
		negative_output = self.featurextractor(negative)
		return anchor_output, positive_output, negative_output
